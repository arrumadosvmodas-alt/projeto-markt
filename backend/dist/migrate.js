...
exports.migrateFromRender = migrateFromRender;
 ...
 */
async function migrateFromRender(sourceUrl, targetUrl) {
    console.log("[MIGRATION] Iniciando migração Render → Railway...");
    const source = new pg_1.Client({ connectionString: sourceUrl, ssl: { rejectUnauthorized: false } });
    const target = new pg_1.Client({ connectionString: targetUrl });
    try {
        ...
        for (const u of users) {
            ...
            await target.query(`INSERT INTO "User" (
        ...
        for (const m of markets) {
            await target.query(`INSERT INTO "Market" (id, name, address, lat, lng, source, "externalId", "createdByUserId", "createdAt")
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
         ON CONFLICT (id) DO NOTHING`, [m.id, m.name, m.address, m.lat, m.lng, m.source, m.externalId, m.createdByUserId, m.createdAt]);
        }
        ...
        for (const p of products) {
            await target.query(`INSERT INTO "Product" (id, barcode, name, brand, category, "imageUrl", source, "createdAt")
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
         ON CONFLICT (id) DO NOTHING`, [p.id, p.barcode, p.name, p.brand, p.category, p.imageUrl, p.source, p.createdAt]);
        }
        ...
        for (const p of purchases) {
            await target.query(`INSERT INTO "Purchase" (
        ...
        for (const i of items) {
            await target.query(`INSERT INTO "PurchaseItem" (id, "purchaseId", "productId", price, quantity, subtotal, "createdAt")
         VALUES ($1,$2,$3,$4,$5,$6,$7)
         ON CONFLICT (id) DO NOTHING`, [i.id, i.purchaseId, i.productId, i.price, i.quantity, i.subtotal, i.createdAt]);
        }
        ...
        for (const l of limits) {
            await target.query(`INSERT INTO "WalletLimit" (
