...
function getCycleDates(cycleDay: number, date: Date = new Date()) {
...
async function getExpensesForPeriod(userId: string, startDate: Date, endDate: Date) {
...
// GET /wallet/status
router.get("/status", getAuthUser, async (req: any, res) => {
  const user = req.user;
  const now = new Date();
  ...
  const activeLimit = await prisma.walletLimit.findFirst({
    where: {
      userId: user.id,
  ...
  const productAggregates: Record<string, { name: string; amount: number }> = {};
...
// POST /wallet/limits
router.post("/limits", getAuthUser, async (req: any, res) => {
  const user = req.user;
  ...
  const prevLimit = await prisma.walletLimit.findFirst({
    where: {
      userId: user.id,
  ...
  const existing = await prisma.walletLimit.findFirst({
    where: {
      userId: user.id,
      cycleStartDate: startDate,
    },
  ...
  let walletLimit;
  if (existing) {
    walletLimit = await prisma.walletLimit.update({
    ...
    walletLimit = await prisma.walletLimit.create({
      data: {
        userId: user.id,
        cycleStartDate: startDate,
        cycleEndDate: endDate,
...
// GET /wallet/history
router.get("/history", getAuthUser, async (req: any, res) => {
  const user = req.user;

  // Get last 6 cycle records
  const limits = await prisma.walletLimit.findMany({
